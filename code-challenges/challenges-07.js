...
const cvFormatter = (arr) => {
    ...
    for(let i = 0; i<arr.length; i++){
        ...
        if(arr[i].yearsOfExperience>1 && (arr[i].firstName||arr[i].lastName)){
            ...
            arr1.push({fullName, tech: arr[i].tech});
        }
...
const applicationsStatics = (arr) => {
        ...
        let result = { // the type of object
            python_devs: 0,
            javaScript_devs: 0,
            dotNet_devs: 0,
            java_devs: 0,
            totalApplicants: arr.length, //to count number in array
            rejectedApplicants: 0,
        ...
        for(let i=0; i<arr.length;i++){
            if((!arr[i].firstName && !arr[i].lastName)|| arr[i].yearsOfExperience<=1){
                ...
                switch(arr[i].tech){
                    case "JS" : result.javaScript_devs++  //we used the name of object + . to access on this att
                    break;
